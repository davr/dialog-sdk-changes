...
static void handle_ble_evt_gap_numeric_request(ble_evt_gap_numeric_request_t *info)
{
        ...
        print_parameter("Connection index: %d", info->conn_idx);
        print_parameter("Numeric key: %06" PRIu32 "", info->num_key);
}
...
static bool gap_adv_data_get(int argc, const char **argv)
{
        ...
        if (adv_len_out > BLE_ADV_DATA_LEN_MAX) {
                adv_len_out = BLE_ADV_DATA_LEN_MAX;
        }

        if (scan_rsp_len_out > BLE_SCAN_RSP_LEN_MAX) {
                scan_rsp_len_out = BLE_SCAN_RSP_LEN_MAX;
        }

        /* Create printable strings */
        memcpy(adv_data_str, format_adv_data(adv_data, adv_len_out), (adv_len_out * 2) + 1);
        adv_data_str[adv_len_out * 2] = '\0';
        memcpy(scan_rsp_data_str, format_adv_data(scan_rsp_data, scan_rsp_len_out), (scan_rsp_len_out * 2) + 1);
        scan_rsp_data_str[scan_rsp_len_out * 2] = '\0';
...
static bool gap_passkey_reply(int argc, const char **argv)
{
        ...
        print_command("ble_gap_passkey_reply ( %d, %s, %06" PRIu32 " )", conn_idx,
...
/*
 * Handler for gap_skip_peripheral_latency.
 * Enable or disable skip latency for a peripheral connection.
 */
#if (dg_configBLE_PERIPHERAL == 1)
#if (dg_configBLE_SKIP_LATENCY_API == 1)
static bool gap_skip_peripheral_latency(int argc, const char **argv)
{
        ble_error_t status;
        uint16_t conn_idx;
        bool enable;
        ...
        status = ble_gap_skip_peripheral_latency(conn_idx, enable);

        print_command("ble_gap_skip_peripheral_latency ( %d, %d )", conn_idx, enable);
        print_status("return: %s", get_status(status));
...
#if (dg_configBLE_PERIPHERAL == 1)
#if (dg_configBLE_SKIP_LATENCY_API == 1)
        { "skip_peripheral_latency", "<conn_idx> <enable>", gap_skip_peripheral_latency },
#endif /* (dg_configBLE_SKIP_LATENCY_API == 1) */
#endif /* (dg_configBLE_PERIPHERAL == 1) */
