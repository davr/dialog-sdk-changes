...
void gpio_wkup_cmd_handler()
{
        if (wkup_gpio.port == HW_GPIO_PORT_MAX || wkup_gpio.pin == HW_GPIO_PIN_MAX) {
                 ...
                 */
                hw_wkup_clear_gpio_status(HW_GPIO_PORT_0, (1 << hw_gpio_port_num_pins[HW_GPIO_PORT_0]) - 1);
                hw_wkup_clear_gpio_status(HW_GPIO_PORT_1, (1 << hw_gpio_port_num_pins[HW_GPIO_PORT_1]) - 1);
                return;
        }

        if (hw_wkup_get_gpio_status(wkup_gpio.port) & (1 << wkup_gpio.pin)) {
                hw_wkup_clear_gpio_status(wkup_gpio.port, (1 << wkup_gpio.pin));
...
static bool wkup_disable()
{
        ...
        hw_wkup_set_trigger(wkup_gpio.port, wkup_gpio.pin, HW_WKUP_TRIG_DISABLED);
        hw_wkup_clear_gpio_status(wkup_gpio.port, (1 << wkup_gpio.pin));
        wkup_gpio.counter = 0;
...
static bool gpio_wkup_enable(int argc, const char **argv)
{
        ...
        if (!strcmp(argv[3], "low")) {
                entry.state = HW_WKUP_TRIG_LEVEL_LO;
        } else if (!strcmp(argv[3], "high")) {
                entry.state = HW_WKUP_TRIG_LEVEL_HI;
        } else {
        ...
        hw_wkup_set_trigger(entry.port, entry.pin, entry.state);
