...
__PACKED_STRUCT boot_report_mouse {
        uint8_t button1 : 1;
        uint8_t button2 : 1;
        uint8_t button3 : 1;
        uint8_t reserved : 5;
...
__PACKED_STRUCT boot_report_keyboard {
        uint8_t mod;
        uint8_t reserved;
        uint8_t key[6];
};

__PACKED_STRUCT boot_report_keyboard_out {
        uint8_t numlock : 1;
        uint8_t capslock : 1;
...
__PACKED_STRUCT report_02_in {
        uint8_t mod;
        uint8_t reserved;
        uint8_t key[6];
};

__PACKED_STRUCT report_02_out {
        uint8_t numlock : 1;
        uint8_t capslock : 1;
        uint8_t scrolllock : 1;
        uint8_t compose : 1;
...
static void sim_keyboard_out(bool capslock)
{
        ...
        if (capslock) {
                hw_led_pwm_set_duty_cycle_pct_off(HW_LED_ID_LED_2, 0, 0);
                hw_led_pwm_set_frequency_hz(LED_PWM_LOWEST_FREQ);
