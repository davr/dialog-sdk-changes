...
__RETAINED static bool stop_before_adv;

__RETAINED static gap_conn_mode_t conn_mode = GAP_CONN_MODE_DIRECTED;

static void start_adv(gap_conn_mode_t mode)
{
        ble_gap_adv_start(mode);
        OS_TIMER_RESET(adv_timer, OS_TIMER_FOREVER);

}

static void apply_adv_par(void)
{
        size_t len;
        gap_device_t dev;
        ble_error_t err;
         ...
         */
        if (is_bonded && conn_mode == GAP_CONN_MODE_DIRECTED) {
                ble_gap_adv_direct_address_set(&dev.address);
        } else {
                conn_mode = GAP_CONN_MODE_UNDIRECTED;
...
void wkup_cb(void)
{
        ...
        if (app_task &&
            (KEY1_MODE == HW_GPIO_MODE_INPUT_PULLUP) ? !hw_gpio_get_pin_status(KEY1_PORT, KEY1_PIN)
                                                     : hw_gpio_get_pin_status(KEY1_PORT, KEY1_PIN)) {
                OS_TASK_NOTIFY_FROM_ISR(app_task, WKUP_TIMER_NOTIF, OS_NOTIFY_SET_BITS);
        }
}
...
void device_task(void *params)
{
         ...
         */
        if ((KEY1_MODE == HW_GPIO_MODE_INPUT_PULLUP) ? !hw_gpio_get_pin_status(KEY1_PORT, KEY1_PIN)
                                                     : hw_gpio_get_pin_status(KEY1_PORT, KEY1_PIN)) {
                gap_device_t dev;
                size_t len = 1;
                ble_error_t err;
        ...
        /* This will apply new parameters and start advertising */
        apply_adv_par();
...
no_event:
                ...
                if (notif & WKUP_TIMER_NOTIF) {
                        if (!peer_state.connected) {
                                conn_mode = GAP_CONN_MODE_DIRECTED;
                                apply_adv_par();
                        }
                }
        }
