...
static void wkup_handler()
{
        ...
        if (!hw_gpio_get_pin_status(KEY1_PORT, KEY1_PIN)) {
                ancs_client_wkup_handler(BUTTON_NEGATIVE);
        } else if (!hw_gpio_get_pin_status(KEY2_PORT, KEY2_PIN)) {
                ancs_client_wkup_handler(BUTTON_POSITIVE);
        }
}
...
static void init_wakeup(void)
{
        ...
        hw_wkup_set_trigger(KEY2_PORT, KEY2_PIN, HW_WKUP_TRIG_LEVEL_LO_DEB);
