...
__RETAINED static ble_service_t *uds;
__RETAINED static ble_service_t *wss;
__RETAINED static ble_service_t *bcs;
...
__RETAINED static user_data_t user_data[CFG_UDS_MAX_USERS];
__RETAINED_RW static uint8_t selected_user_id = UDS_USER_ID_UNKNOWN;
...
{
        if (current_task) {
                OS_TASK_NOTIFY_FROM_ISR(current_task, WSS_MEASUREMENT_NOTIF, OS_NOTIFY_SET_BITS);
        }
}
...
{
        OS_TASK_NOTIFY_FROM_ISR(current_task, BCS_MEASUREMENT_NOTIF, OS_NOTIFY_SET_BITS);
}

static user_data_t *get_user(uint8_t user_id, bool enable_print)
{
        user_data_t *user;

        if (user_id == UDS_USER_ID_UNKNOWN) {
                if (enable_print) {
                        printf("ERROR: user %u is invalid\r\n", user_id);
                }
                return NULL;
        }
        ...
        if (user_id >= CFG_UDS_MAX_USERS) {
                if (enable_print) {
                        printf("ERROR: user id out of scope\r\n");
                }
                return NULL;
        }
        ...
        if (!user->used) {
                if (enable_print) {
                        printf("ERROR: user %u is not registered\r\n", user_id);
                }
                return NULL;
        }
...
{
        user_data_t *user = get_user(user_id, false);
...
static void uds_db_read_cb(ble_service_t *svc, uint16_t conn_idx, uint32_t field)
{
        ...
        if (user_id == UDS_USER_ID_UNKNOWN || user_id >= CFG_UDS_MAX_USERS) {
...
static void handle_evt_gap_disconnected(ble_evt_gap_disconnected_t *evt)
{
        ...
        /*
         * Always try start advertising when disconnected. If a single connection is supported,
         * application advertises only when no device is connected. In case of multiple connections
         * support, it will usually fail (as advertising is already started) except we reached
         * maximum connected devices - advertising is stopped then, so we restart it here.
         */
...
{
        if (evt->status != BLE_ERROR_NOT_ALLOWED) {
                ble_gap_adv_start(GAP_CONN_MODE_UNDIRECTED);
        }
}
#endif
...
static void cmd_user_register_handler(int argc, const char *argv[])
{
        ...
        if (!parse_u16(argv[2], &consent)) {
                printf("ERROR: invalid format of <consent>\r\n");
                return;
        }

        if (consent > CFG_UDS_MAX_VALID_USER_CONSENT) {
                printf("ERROR: invalid consent, valid are <0...%u>\r\n", CFG_UDS_MAX_VALID_USER_CONSENT);
                return;
        }

        user_id = user_reg_helper(consent, (argc > 3) ? argv[3] : NULL);
        if (user_id == UDS_USER_ID_UNKNOWN) {
                printf("ERROR: all users IDs are occupied\r\n");
                return;
        }
...
static void cmd_user_remove_handler(int argc, const char *argv[])
{
        ...
        if (argc != 3) {
                printf("usage: user remove <user_id | all>\r\n");
                return;
        }
        ...
        if (!parse_u8(argv[2], &id)) {
                printf("ERROR: invalid user id\r\n");
                return;
        }

        if (get_user(id, true) == NULL) {
                return;
        }
...
static void cmd_user_change_name_handler(int argc, const char *argv[])
{
        size_t size;
        char *name;
        uint8_t id;
        ...
        if (argc != 4) {
                printf("usage: user name <id> <new_name>\r\n");
                return;
        }
        ...
        printf("User %u has been changed\r\n", id);
}

static void cmd_user_change_age_handler(int argc, const char *argv[])
{
        uint8_t id;
        uint8_t age;

        if (argc != 4) {
                printf("usage: user age <id> <new_age>\r\n");
                return;
        }
        ...
        if (!parse_u8(argv[3], &age)) {
                printf("ERROR: invalid user's new age\r\n");
                return;
        }

        user_data[id].age = age;
        write_user_to_storage(id);
        printf("User %u has been changed\r\n", id);
}

static void cmd_user_change_height_handler(int argc, const char *argv[])
{
        uint8_t id;
        uint8_t height;

        if (argc != 4) {
                printf("usage: user height <id> <new_height>\r\n");
                return;
        }
        ...
        if (!parse_u8(argv[3], &height)) {
                printf("ERROR: invalid user's new height\r\n");
                return;
        }

        user_data[id].height = height;
        write_user_to_storage(id);
        printf("User %u has been changed\r\n", id);
}

static void cmd_user_change_gender_handler(int argc, const char *argv[])
{
        uint8_t id;

        if (argc != 4) {
                printf("usage: user gender <id> <gender>\r\n");
                return;
        }
        ...
        if (!strcmp(argv[3], "male")) {
                user_data[id].gender = UDS_GENDER_MALE;
        } else if (!strcmp(argv[3], "female")) {
...
static void cmd_user_change_date_of_birth_handler(int argc, const char *argv[])
{
        int day, month, year;
        int res;
        uint8_t id;
        ...
        if (argc != 4) {
                printf("usage: user birth <id> <dd-mm-yyyy>\r\n");
                return;
        }
        ...
        res = sscanf(argv[3], "%2d-%2d-%4d", &day, &month, &year);
        if (res != 3) {
                printf("ERROR: invalid birth date format, usage: user birth <id> <dd-mm-yyyy>\r\n");
                return;
        }

        if (!uds_date_validation_helper(year, month, day)) {
                printf("ERROR: date range is not valid\r\n");
                return;
        }
...
{
        printf("Usage: user register <consent> [first_name]\r\n");
        printf("       user remove <id | all>\r\n");
        printf("       user list\r\n");
        printf("       user info <id>\r\n");
        printf("       user select <id>\r\n");
        printf("       user name <id> <new_name>\r\n");
        printf("       user age <id> <new_age>\r\n");
        printf("       user height <id> <new_height>\r\n");
        printf("       user gender <id> <male | female>\r\n");
        printf("       user birth <id> <dd-mm-yyyy>\r\n");
}
...
static void cmd_user_handler(int argc, const char *argv[], void *data)
{
        ...
        if (!strcmp(argv[1], "register")) {
                ...
                cmd_user_change_name_handler(argc, argv);
        } else if (!strcmp(argv[1], "age")) {
                cmd_user_change_age_handler(argc, argv);
        } else if (!strcmp(argv[1], "height")) {
                cmd_user_change_height_handler(argc, argv);
        } else if (!strcmp(argv[1], "gender")) {
                cmd_user_change_gender_handler(argc, argv);
        } else if (!strcmp(argv[1], "birth")) {
                cmd_user_change_date_of_birth_handler(argc, argv);
        } else {
                cmd_user_usage();
        }
}
...
static void cmd_time_set_handler(int argc, const char *argv[])
{
        ...
        res = sscanf(argv[2], "%2d-%2d-%4d", &day, &month, &year);
        if (res != 3) {
                printf("ERROR: incorrect date format, usage: time set <dd-mm-yyyy> <hh:mm:ss>\r\n");
                return;
        }

        res = sscanf(argv[3], "%2d:%2d:%2d", &hour, &min, &sec);
        if (res != 3) {
                printf("ERROR: incorrect time format, usage: time set <dd-mm-yyyy> <hh:mm:ss>\r\n");
                return;
        }
        ...
        if (!((hour >= 0 && hour < 24) && (min >= 0 && min < 60) && (sec >= 0 && sec < 60))) {
                printf("ERROR: time range is not valid\r\n");
                return;
        }
...
static void cmd_time_usage(void)
{
        ...
        printf("       time set <dd-mm-yyyy> <hh:mm:ss>\r\n");
}
...
OS_TASK_FUNCTION(wsp_weightscale_task, params)
{
        ...
        bcs_meas_tmr = OS_TIMER_CREATE("bcs_meas", OS_TIME_TO_TICKS(5000), OS_TIMER_RELOAD,
         ...
         */
        cts_timer = OS_TIMER_CREATE("cts", OS_MS_2_TICKS(1000), OS_TIMER_RELOAD,
                                                                        current_task, cts_timer_cb);
        OS_ASSERT(cts_timer);
        OS_TIMER_START(cts_timer, OS_TIMER_FOREVER);


        /* Add Dialog Debug Service */
