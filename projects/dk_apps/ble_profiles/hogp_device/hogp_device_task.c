...
/* Control Point Value */
__RETAINED static hids_cp_command_t cp_state;
/* Protocol mode value */
__RETAINED static hids_protocol_mode_t protocol_mode;
/* Timer used for cursor movement */
__RETAINED static OS_TIMER hid_timer;
/* Values used to calculate report value */
__RETAINED static int32_t side_length;
__RETAINED static uint8_t side;
__RETAINED static OS_TASK current_task;
__RETAINED static int reports_depth;
...
static void hid_timer_cb(OS_TIMER timer)
{
        ...
        OS_TASK_NOTIFY(task, HID_TIMER_NOTIF, OS_NOTIFY_SET_BITS);
}
#endif
...
OS_TASK_FUNCTION(hogp_device_task, params)
{
        int8_t wdog_id;
        ...
        hid_timer = OS_TIMER_CREATE("hid", OS_MS_2_TICKS(MOUSE_TIMEOUT), OS_TIMER_SUCCESS,
