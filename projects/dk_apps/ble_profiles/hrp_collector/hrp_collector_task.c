...
#define GATT_CLIENT_STORAGE_ID  BLE_STORAGE_KEY_APP(0, 2)
...
typedef enum {
        ...
        PENDING_ACTION_ENABLE_SVC_CHANGED_IND = (1 << 7),
} pending_action_t;
...
static void purge_hrs(peer_info_t *peer_info)
{
...
static void purge_clients_in_range(peer_info_t *peer_info, uint16_t start_h, uint16_t end_h)
{
        ...
        if (peer_info->hrs_client && ble_client_in_range(peer_info->hrs_client, start_h,
                                                                                end_h)) {
                purge_hrs(peer_info);
        }
...
void hrs_set_event_state_completed_cb(ble_client_t *client, hrs_client_event_t event, att_error_t status)
{
        ...
        /*
         * Only Heart Rate measurements notifications are supported.
         */
...
static void handle_evt_gap_connected(const ble_evt_gap_connected_t *evt)
{
        ...
        /*
         * Try to start encryption first. If it fails it means we are not bonded and can
...
static void handle_evt_gattc_browse_completed(const ble_evt_gattc_browse_completed_t *evt)
{
        ...
        if (peer_info->gatt_client) {
                ...
                /* Register for Service Changed indication */
...
OS_TASK_FUNCTION(hrp_collector_task, params)
{
        cli_t cli;
