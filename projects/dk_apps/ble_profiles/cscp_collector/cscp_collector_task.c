...
#define GATT_CLIENT_STORAGE_ID    BLE_STORAGE_KEY_APP(0, 3)
#define MEASUREMENT_STORAGE_ID    BLE_STORAGE_KEY_APP(0, 4)
...
typedef enum {
        ...
        PENDING_ACTION_ENABLE_SVC_CHANGED_IND = (1 << 8),
} pending_action_t;
...
static void purge_cscs(peer_info_t *peer_info)
{
...
{
        if (peer_info->gatt_client && ble_client_in_range(peer_info->gatt_client, start_h,
                                                                                end_h)) {
                purge_gatt(peer_info);
        }

        if (peer_info->cscs_client && ble_client_in_range(peer_info->cscs_client, start_h,
                                                                                end_h)) {
                purge_cscs(peer_info);
        }

        if (peer_info->dis_client && ble_client_in_range(peer_info->dis_client, start_h,
                                                                                end_h)) {
                purge_dis(peer_info);
        }
}
...
static void finish_auth(peer_info_t *peer_info, ble_error_t status)
{
        ...
        if (status == BLE_ERROR_ENC_KEY_MISSING) {
                ...
                purge_clients_in_range(peer_info, 0x0001, 0xffff);
...
static void clicmd_scan_usage(void)
{
        ...
        printf("\t\"any\" will disable filtering devices by CSCS UUID, only valid for \"scan start\"\r\n");
}
...
static void clicmd_disconnect_handler(int argc, const char *argv[], void *user_data)
{
        ...
        if (!parse_u16(argv[1], &conn_idx)) {
                printf("ERROR: invalid connection index format\r\n");
                return;
        }
...
static void clicmd_notifications_handler(int argc, const char *argv[], void *user_data)
{
        ...
        if (!peer_info) {
                printf("ERROR: device has to be connected to turn on/off notifications\r\n");
                return;
        }
...
static void clicmd_get_handler(int argc, const char *argv[], void *user_data)
{
        ...
        if (!peer_info) {
                printf("ERROR: device has to be connected to get location/locations\r\n");
                return;
        }

        if (!strcmp("locations", argv[2])) {
        ...
        } else if (!strcmp("location", argv[2])) {
...
static void clicmd_set_handler(int argc, const char *argv[], void *user_data)
{
        ...
        if (!peer_info) {
                printf("ERROR: device has to be connected to set location/locations\r\n");
                return;
        }

        if (!strcmp("wheelrev", argv[2])) {
                uint32_t cum_val = 0;

                if (!parse_u32(argv[3], &cum_val)) {
                        printf("ERROR: invalid cumulative value\r\n");
                        return;
                }
                ret = cscs_client_set_cumulative_value(peer_info->cscs_client, cum_val);
        } else if (!strcmp("location", argv[2])) {
                uint8_t sensor_loc;

                if (!parse_u8(argv[3], &sensor_loc)) {
                        printf("ERROR: invalid sensor location\r\n");
                        return;
                }
                ret = cscs_client_update_sensor_location(peer_info->cscs_client, sensor_loc);
        } else {
                clicmd_set_usage();
                return;
        }
...
} cscs_measurement_serialized_t;

static void store_last_measurement(peer_info_t *peer_info)
{
        cscs_measurement_serialized_t *measurement;

        measurement = OS_MALLOC(sizeof(*measurement));
        measurement->wheel_revolution_data_present = peer_info->last_measurement.wheel_revolution_data_present;
        measurement->crank_revolution_data_present = peer_info->last_measurement.crank_revolution_data_present;
        measurement->cumulative_crank_revolutions = peer_info->last_measurement.cumulative_crank_revolutions;
        measurement->last_crank_event_time = peer_info->last_measurement.last_crank_event_time;
        measurement->cumulative_wheel_revolutions = peer_info->last_measurement.cumulative_wheel_revolutions;
        measurement->last_wheel_event_time = peer_info->last_measurement.last_wheel_event_time;

        ble_storage_put_buffer(peer_info->conn_idx, MEASUREMENT_STORAGE_ID, sizeof(*measurement),
                                                                measurement, OS_FREE_FUNC, true);
}

static void restore_last_measurement(peer_info_t *peer_info)
{
        cscs_measurement_serialized_t *measurement;
        ble_error_t err;
        uint16_t len = 0;
        ...
        err = ble_storage_get_buffer(peer_info->conn_idx, MEASUREMENT_STORAGE_ID, &len, &buffer);
        if (err) {
                return;
        }

        if (len != sizeof(cscs_measurement_serialized_t)) {
                return;
        }

        measurement = buffer;
        peer_info->last_measurement.wheel_revolution_data_present = measurement->wheel_revolution_data_present;
        peer_info->last_measurement.crank_revolution_data_present = measurement->crank_revolution_data_present;
        peer_info->last_measurement.cumulative_crank_revolutions = measurement->cumulative_crank_revolutions;
        peer_info->last_measurement.last_crank_event_time = measurement->last_crank_event_time;
        peer_info->last_measurement.cumulative_wheel_revolutions = measurement->cumulative_wheel_revolutions;
        peer_info->last_measurement.last_wheel_event_time = measurement->last_wheel_event_time;
}
...
static void cscs_csc_measurement_cb(ble_client_t *client, const cscs_client_measurement_t *measurement)
{
        ...
        if (measurement->wheel_revolution_data_present) {
                ...
                uint32_t tmp = time * 1000 / 1024;

                printf("\tLast wheel event time: %" PRIu16 ".%03" PRIu16 " [s] (raw %" PRIu16 ")\r\n",
                                        (uint16_t)(time / 1024), (uint16_t)(tmp % 1000), (uint16_t)time);
        ...
        store_last_measurement(peer_info);
}
...
static void handle_evt_gap_connected(const ble_evt_gap_connected_t *evt)
{
        ...
        if (bonded) {
                ...
                restore_last_measurement(peer_info);
        }
...
static void handle_evt_gattc_browse_completed(const ble_evt_gattc_browse_completed_t *evt)
{
        ...
        if (queue_length(&peer_info->pending_browse_queue)) {
                ...
                purge_clients_in_range(peer_info, req->start_h, req->end_h);
...
OS_TASK_FUNCTION(cscp_collector_task, params)
{
        cli_t cli;
